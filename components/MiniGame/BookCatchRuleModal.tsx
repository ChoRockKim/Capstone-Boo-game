/**
 * @description  전공책 받기 미니게임의 규칙과 점수표를 표시하는 설명 모달입니다.
 * @depends      assets/icons/cross.svg, assets/miniGame/book-catch/*, constants/colors.ts, constants/fonts.ts, expo-image, utils/soundEffects.ts
...
type BookCatchRuleModalProps = {
  onClose: () => void;
};

const BookCatchRuleModal = ({ onClose }: BookCatchRuleModalProps) => {
  const handleClose = () => {
    ...
    <View pointerEvents="auto" style={styles.overlay}>
      <View style={styles.card}>
        ...
        <Text style={styles.descriptionText}>
          화살표를 이용해 부를 좌우로 움직이며{"\n"}전/공/책을 받으세요!
        </Text>

        <Text style={styles.sectionTitleText}>점수 안내</Text>
        <View style={styles.scoreGrid}>
          {BOOK_SCORE_ITEMS.map((item) => (
            <View key={item.point} style={styles.scoreItem}>
              <Image
        ...
        <Text style={styles.tipText}>
          일반모드: 종료 5초 전부터 더 빨라져요.{"\n"}무한모드: 방해물 3번이면 종료, 책마다 속도 +1%.
        </Text>
      </View>
    </View>
  );
...
export default BookCatchRuleModal;
